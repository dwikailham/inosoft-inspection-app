...
export function OrderItemField({ itemIndex, remove, control, register, dropdowns, totalItems }: any) {
  ...
  React.useEffect(() => {
    if (prevItemCode.current !== undefined && prevItemCode.current !== selectedItemCode) {
      ...
      replace([{ lot_no: '', allocation: '', owner: '', condition: '', avail_qty: 0, qty_required: 0, inspection_required: false }]);
    }
    ...
    <div className="border border-neutral-200 bg-neutral-50 p-4 rounded-md mb-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end mb-4 border-b pb-4 space-y-4 sm:space-y-0">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full lg:w-2/3">
          <RHFSelect
            ...
            options={dropdowns.itemCodes.map((i: any) => ({ id: i.id, name: i.code + ' - ' + i.desc }))}
      ...
      <div className="space-y-4">
        {fields.map((lotField, lotIndex) => {
          ...
          if (selectedItemCode) {
            const codeRef = dropdowns.itemCodes.find((i: any) => i.id === selectedItemCode)?.code;
            availableStocks = availableStocks.filter((s: any) => s.code === codeRef);
          }

          // If lot_no is selected, filter available allocations, etc.
          let allocations = [...new Set(availableStocks.filter((s: any) => !currentLotVal.lot_no || s.lot_no === currentLotVal.lot_no).map((s: any) => s.allocation))];
          let owners = [...new Set(availableStocks.filter((s: any) => (!currentLotVal.lot_no || s.lot_no === currentLotVal.lot_no) && (!currentLotVal.allocation || s.allocation === currentLotVal.allocation)).map((s: any) => s.owner))];
          let conditions = [...new Set(availableStocks.filter((s: any) => (!currentLotVal.lot_no || s.lot_no === currentLotVal.lot_no) && (!currentLotVal.allocation || s.allocation === currentLotVal.allocation) && (!currentLotVal.owner || s.owner === currentLotVal.owner)).map((s: any) => s.condition))];

          // Re-calculate Avail Qty if all exact match
          const exactStock = availableStocks.find((s: any) => s.lot_no === currentLotVal.lot_no && s.allocation === currentLotVal.allocation && s.owner === currentLotVal.owner && s.condition === currentLotVal.condition);
          ...
          return (
            <div key={lotField.id} className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-4 lg:gap-2 items-end mb-6 pb-4 border-b lg:mb-0 lg:pb-0 lg:border-none">
              ...
              <div className="flex flex-col items-start sm:items-center justify-center space-y-1.5 sm:pb-2">
                ...
                <input disabled={!selectedItemCode} type="checkbox" {...register(`items.${itemIndex}.lots.${lotIndex}.inspection_required`)} className="h-4 w-4" />
              </div>
      ...
      <Button disabled={!selectedItemCode} type="button" variant="outline" size="sm" className="mt-4" onClick={() => append({ lot_no: '', allocation: '', owner: '', condition: '', avail_qty: 0, qty_required: 0, inspection_required: false })}>
